...
def job_master_data(cfg):
 ...
 #dblist=db_connection.list_database_names()
  ...
  # job_date_created
  # job_user_created
  df =sqlC.sql("SELECT ROW_NUMBER() OVER(ORDER BY (SELECT NULL)) AS job_id,job_id as job_post_id,company as job_cmp_name,location as job_country,place as job_place, seniority_level as job_seniority_level,job_function,link as job_apply_link,date as job_date_posted FROM data")
