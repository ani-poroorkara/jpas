...
def job_type_data(cfg):
  logging.info("Creating Database connection for type summarization")
 ...
 #dblist=db_connection.list_database_names()
  ...
  df =sqlC.sql("SELECT count(*) as job_count,job_type from data group by job_type")
